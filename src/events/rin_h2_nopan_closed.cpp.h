#ifndef RIN_H2_NOPAN_CLOSED_CPP_H
#define RIN_H2_NOPAN_CLOSED_CPP_H
...
class RinH2NopanClosedEvent final : public CustomEvent {
public:
    RinH2NopanClosedEvent() : CustomEvent(DISPLAYABLE_BITMASK_NONE) {}

    [[nodiscard]] bn::unique_ptr<CustomEvent> create() const override {
        return bn::make_unique<RinH2NopanClosedEvent>();
    }

    [[nodiscard]] bn::optional<bn::fixed_point> background_target_position() const override {
        return rin_h2_motion::position_for_yalign(0.8);
    }

    void init() override {
        BN_ASSERT(background_visual.visible_bg_item.has_value(), "Event background is not set");
        const bn::fixed_point pos = rin_h2_motion::position_for_yalign(0.8);
...
#endif  // RIN_H2_NOPAN_CLOSED_CPP_H
