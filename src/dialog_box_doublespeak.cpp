...
namespace ks {
    ...
    void dialog_box_doublespeak::show(const bool blending) {
        ...
        for (int actor_num = 0; actor_num < 2; actor_num++) {
            ...
            const int title_ends_x = -device::screen_width_half + 8 + _default_text_generator->width(_actor->name()) + offset_x;
            ...
            _default_text_generator->generate(-ks::device::screen_width_half + 8 + offset_x, ks::device::screen_height_half - 52,
